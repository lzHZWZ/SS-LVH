...
class GCNMultiLabelHashEngine(GCNMultiLabelMAPEngine):
	...
	def on_forward(self, training, model, criterion, data_loader, optimizer=None, display=True):
		...
		target_var = torch.autograd.Variable(self.state['target']).float()
		inp_var = torch.autograd.Variable(self.state['input']).float().detach()
		cor_var = torch.autograd.Variable(self.state['correlate']).float().detach()
		if not training:
			with torch.no_grad():
				feature_var = torch.autograd.Variable(self.state['feature']).float()
				target_var = torch.autograd.Variable(self.state['target']).float()
				inp_var = torch.autograd.Variable(self.state['input']).float()
				cor_var = torch.autograd.Variable(self.state['correlate']).float()
		if training:
			gl.LOCAL_USE_TANH = False
		else:
			gl.LOCAL_USE_TANH = True
		
		self.state['output'] = model(feature_var, inp_var, cor_var)
		self.state['loss'] = criterion(self.state['target'], self.state['output'])
	...
	def on_end_batch(self, training, model, criterion, data_loader, optimizer=None, display=True):
		...
		if type(loss)==type(1) or type(loss)==type(1.1) or type(loss)==type(1.111111111111111111):
		...
		if display:
			if training:
				...
				print('Training Epoch({0}):\taverage Loss in this epoch is {loss}'. \
				      format(self.state['epoch'], loss=loss))
	...
	def learning(self, model, criterion, train_dataset, val_dataset, optimizer=None):
		...
		for epoch in range(self.state['start_epoch'], self.state['max_epochs']):
			...
			print("At epoch({0}) the now_score is: {1},\tis_best = {2}".format(epoch, now_score, is_best))
			self.state['best_score'] = min(now_score, self.state['best_score'])
			print("At epoch({0}) the self.state['best_score'] is: {1}".format(epoch, self.state['best_score']))
			...
			if self._state('save_model_path') is not None:
				...
				filename_best = os.path.join(self.state['save_model_path'],
				                             ...
				                             format(score=state['best_score'],
				                                    time=self.state["start_time_str"]))
				shutil.copyfile(filename, filename_best)
				self.state['filename_previous_best'] = filename_best
